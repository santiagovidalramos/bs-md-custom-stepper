...
public class CustomStepperItemView extends FrameLayout {
    /**
     * Internal Views
     */
     ...
     * The bind views
     */
    private @Nullable CustomStepperItemView mPrevItemView, mNextItemView;

    public static final int STATE_NORMAL = 0, STATE_SELECTED = 1, STATE_DONE = 2;

    private final int DP;

    public CustomStepperItemView(@NonNull Context context) {
        this(context, null);
    }

    public CustomStepperItemView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomStepperItemView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CustomStepperItemView,
                    defStyleAttr, R.style.Widget_Stepper);

            mTitle = a.getString(R.styleable.CustomStepperItemView_step_title);
            mSummary = a.getString(R.styleable.CustomStepperItemView_step_summary);
            mSummaryFinished = a.getString(R.styleable.CustomStepperItemView_step_summary_done);
            mIndex = a.getInt(R.styleable.CustomStepperItemView_step_index, 1);
            mState = a.getInt(R.styleable.CustomStepperItemView_step_state, STATE_NORMAL);
            isLastStep = a.getBoolean(R.styleable.CustomStepperItemView_step_is_last, false);
            mNormalColor = a.getColor(R.styleable.CustomStepperItemView_step_normal_color, mNormalColor);
            mActivatedColor = a.getColor(R.styleable.CustomStepperItemView_step_activated_color, mActivatedColor);
            mAnimationDuration = a.getInt(R.styleable.CustomStepperItemView_step_animation_duration, mAnimationDuration);
            mAnimationEnabled = a.getBoolean(R.styleable.CustomStepperItemView_step_enable_animation, true);
            mLineColor = a.getColor(R.styleable.CustomStepperItemView_step_line_color, mLineColor);
            mErrorColor = a.getColor(R.styleable.CustomStepperItemView_step_error_highlight_color, mErrorColor);
            mAlwaysShowSummary = a.getBoolean(R.styleable.CustomStepperItemView_step_show_summary_always, mAlwaysShowSummary);

            if (a.hasValue(R.styleable.CustomStepperItemView_step_done_icon)) {
                mDoneIcon = a.getDrawable(R.styleable.CustomStepperItemView_step_done_icon);
            }
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams layoutParams) {
        if (child.getId() == R.id.custom_stepper_item_view_layout) {
    ...
    private void prepareViews(@NonNull Context context) {
        ...
        View inflateView = inflater.inflate(R.layout.custom_stepper_item_view_layout, null);
     ...
     */
    public void bindSteppers(@Nullable CustomStepperItemView prevItem, @Nullable CustomStepperItemView nextItem) {
        if (prevItem != null) {
     ...
     */
    public static void bindSteppers(@NonNull CustomStepperItemView... items) {
